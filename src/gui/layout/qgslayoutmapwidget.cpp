  ...
  , mMapItem( map )
{
  ...
  mClippingTypeComboBox->addItem( tr( "Clip During Render Only" ), static_cast< int >( QgsMapClippingRegion::FeatureClippingType::ClipPainterOnly ) );
  ...
  connect( mClipToAtlasCheckBox, &QGroupBox::toggled, this, [ = ]( bool active )
  {
    if ( !mBlockUpdates )
    {
      ...
      mMapItem->mapClippingSettings()->setAtlasEnabled( active );
      mMapItem->endCommand();
    }
  } );
  ...
  connect( mClippingTypeComboBox, qOverload<int>( &QComboBox::currentIndexChanged ), this, [ = ]
  {
    if ( !mBlockUpdates )
    {
  ...
  connect( mRadioClipSelectedLayers, &QCheckBox::toggled, this, [ = ]( bool active )
  {
    if ( active && !mBlockUpdates )
    {
      ...
      mMapItem->mapClippingSettings()->setRestrictToLayers( true );
      mMapItem->endCommand();
      mBlockUpdates = false;
    }
  } );
  ...
  connect( mRadioClipAllLayers, &QCheckBox::toggled, this, [ = ]( bool active )
  {
    if ( active && !mBlockUpdates )
    {
      mBlockUpdates = true;
      mMapItem->beginCommand( tr( "Change Atlas Clipping Layers" ) );
      mMapItem->mapClippingSettings()->setRestrictToLayers( false );
      mMapItem->endCommand();
      mBlockUpdates = false;
    }
  } );
  connect( mLayerModel, &QgsMapLayerModel::dataChanged, this, [ = ]( const QModelIndex &, const QModelIndex &, const QVector<int> &roles = QVector<int>() )
  {
    ...
    if ( !mBlockUpdates )
    {
      mBlockUpdates = true;
      mMapItem->beginCommand( tr( "Change Atlas Clipping Layers" ) );
      mMapItem->mapClippingSettings()->setLayersToClip( mLayerModel->layersChecked() );
      mMapItem->endCommand();
      mBlockUpdates = false;
    }
  } );
  ...
  connect( mClipToItemCheckBox, &QGroupBox::toggled, this, [ = ]( bool active )
  {
    if ( !mBlockUpdates )
    {
      ...
      mMapItem->mapClippingSettings()->setLayoutItemEnabled( active );
      mMapItem->endCommand();
    }
  } );
  connect( mClipItemComboBox, &QgsLayoutItemComboBox::itemChanged, this, [ = ]( QgsLayoutItem * item )
  {
    if ( !mBlockUpdates )
    {
      ...
      mMapItem->mapClippingSettings()->setSourceItem( item );
      mMapItem->endCommand();
    }
  } );
...
void QgsLayoutMapClippingWidget::setReportTypeString( const QString &string )
{
  ...
  mForceLabelsInsideCheckBox->setText( tr( "Force labels inside cliping extent" ).arg( string ) );
}
...
void QgsLayoutMapClippingWidget::updateGuiElements()
{
  ...
  mBlockUpdates = true;
  mClipToAtlasCheckBox->setChecked( mMapItem->mapClippingSettings()->atlasEnabled() );
  mClippingTypeComboBox->setCurrentIndex( mClippingTypeComboBox->findData( static_cast< int >( mMapItem->mapClippingSettings()->featureClippingType() ) ) );
  mForceLabelsInsideCheckBox->setChecked( mMapItem->mapClippingSettings()->forceLabelsInsideFeature() );

  mRadioClipAllLayers->setChecked( !mMapItem->mapClippingSettings()->restrictToLayers() );
  mRadioClipSelectedLayers->setChecked( mMapItem->mapClippingSettings()->restrictToLayers() );
  mLayerModel->setLayersChecked( mMapItem->mapClippingSettings()->layersToClip() );

  mClipToItemCheckBox->setChecked( mMapItem->mapClippingSettings()->layoutItemEnabled() );
  mClipItemComboBox->setItem( mMapItem->mapClippingSettings()->sourceItem() );
...
void QgsLayoutMapClippingWidget::mGeometryExpressionButton_clicked()
{
  ...
  const QgsExpressionContext context = layout->createExpressionContext();
  QgsExpressionBuilderDialog exprDlg( layoutAtlas()->coverageLayer(), mClipGeometryEdit->text(), this, QStringLiteral( "generic" ), context );
  exprDlg.setWindowTitle( tr( "Expression Based Clipping Geometry" ) );

  if ( exprDlg.exec() == QDialog::Accepted )
  {
    const QString expression = exprDlg.expressionText();
    mClipGeometryEdit->setText( expression );
    mMapItem->refresh();
  }
}
