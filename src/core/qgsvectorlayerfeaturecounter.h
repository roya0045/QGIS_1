...
class CORE_EXPORT QgsVectorLayerFeatureCounter : public QgsTask
{
  ...
  public:
     ...
     * \param withFids If TRUE will store the fids, otherwise will only count the number of features per symbol. Default FALSE.
     */
    QgsVectorLayerFeatureCounter( QgsVectorLayer *layer, const QgsExpressionContext &context = QgsExpressionContext(), bool withFids = false );
